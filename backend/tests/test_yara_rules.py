...
def test_drinik_named_sample_is_still_caught():
    apk = b"classes.dex drinikapk com.example.income_tax accessibilityservice"
...
def test_drinik_needs_more_than_an_incidental_substring():
    ...
    assert "Drinik_Banking_Trojan" not in _scan_bytes(b"UIAssistant helper module" * 30, ".exe")
...
def test_ransomware_shape_still_caught_but_crypto_alone_is_not():
    ...
    ransom = b"CryptEncrypt DeleteFile FindFirstFileW FindNextFileW"
    assert "File_Encryption_API" in _scan_bytes(ransom, ".exe")
    assert "File_Encryption_API" not in _scan_bytes(b"CryptEncrypt DeleteFileW", ".exe")
...
def test_donation_and_backup_wording_is_not_a_ransom_note():
    ...
    app = (b"Support this app: donate via Bitcoin or PayPal. "
